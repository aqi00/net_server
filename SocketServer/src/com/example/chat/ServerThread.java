...
public class ServerThread implements Runnable {
		...
		try {
			...
			while ((content = mReader.readLine()) != null) {
				...
				if (action.equals("LOGIN")) {
					...
					sendmsg("RECVMSG", splitArray[2], splitArray[4],
							splitArray[1], body);
				} else if (action.equals("GETLIST")) {
					getlist(splitArray[1]);
				} else if (action.equals("SENDPHOTO")) {
					sendmsg("RECVPHOTO", splitArray[2], splitArray[4],
							splitArray[1], body);
				} else if (action.equals("SENDSOUND")) {
					sendmsg("RECVSOUND", splitArray[2], splitArray[4],
							splitArray[1], body);
				}
			}
		} catch (Exception e) {
	...
	private void login(String deviceId, String nickName, String loginTime)
			throws IOException {
	...
	private String getFriend() {
		...
		for (SocketBean item : ChatServer.mSocketList) {
			if (item.deviceId != null && item.deviceId.length() > 0) {
				String friend = String.format("|%s,%s,%s", item.deviceId,
						item.nickName, item.loginTime);
				friends += friend;
			}
		}
		return friends;
	...
	private void sendmsg(String respAction, String otherName, String otherId,
			String selfId, String message) throws IOException {
		for (int i = 0; i < ChatServer.mSocketList.size(); i++) {
			SocketBean item = ChatServer.mSocketList.get(i);
			if (item.deviceId.equals(otherId)) {
				String content = String.format("%s,%s,%s,%s|%s", respAction,
						selfId, otherName, DateUtil.getNowTime(), message);
