...
import com.servlet.office.util.DocUtil;
import com.servlet.office.util.PptUtil;
import com.servlet.office.util.XlsUtil;
...
public class ParseDoc extends HttpServlet {
                ...
                if (item.isFormField()) { // 如果传过来的是普通的表单域
                    ...
                    if (item.getContentType().startsWith("application/*")) { // 支持ppt和pptx
                        ...
                        if (fileName.endsWith("doc") || fileName.endsWith("docx")) {
                            String filePath = FileUtil.saveFile(item, "doc/");
                            htmlPath = DocUtil.toHtml(filePath);
                        } else if (fileName.endsWith("xls") || fileName.endsWith("xlsx")) {
                            String filePath = FileUtil.saveFile(item, "xls/");
                            htmlPath = XlsUtil.toHtml(filePath);
                        }
                        parseResponse.setHtmlPath(htmlPath);
                    }
                }
            }
        } else {
