...
public class XlsUtil {
    ...
    public static String toHtml(String relativePath) {
        ...
        int destPos = destPath.lastIndexOf("/");
        String destDir = destPath.substring(0, destPos+1);
        if (!new File(destDir).exists()) {
            new File(destDir).mkdirs();
        }
        StringBuilder stringBuilder = new StringBuilder();
		try (FileOutputStream fos = new FileOutputStream(destPath)) {
			XlsUtil excelToHtml = XlsUtil.create(srcPath, stringBuilder);
    ...
    public static XlsUtil create(Workbook wb, Appendable output) {
        return new XlsUtil(wb, output);
    }

    public static XlsUtil create(String path, Appendable output)
            throws IOException {
        return create(new FileInputStream(path), output);
    }

    public static XlsUtil create(InputStream in, Appendable output)
            throws IOException {
        Workbook wb = WorkbookFactory.create(in);
        return create(wb, output);
    }

    private XlsUtil(Workbook wb, Appendable output) {
        if (wb == null) {
